...
Radiant is deployed on the following stack.
...
documented on that page, you need to create a new file in ``radiant/celery.py`` and then
modify ``radiant/__init__.py`` to import that file. You'll also need to customize ``{{
...
``requirements/base.txt`` and installed it)::
    ...
    celery -A radiant worker

Additionally you will need to uncomment the ``BROKER_URL`` setting in the project settings::

    # radiant/settings/deploy.py
    from .base import *

    # ...
    BROKER_URL = 'amqp://radiant_%(ENVIRONMENT)s:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/radiant_%(ENVIRONMENT)s' % os.environ
...
the broker by default. The RabbitMQ user will be named ``radiant_<environment>`` and the
vhost will be named ``radiant_<environment>`` for each environment. It requires that you
...
You can use the below OpenSSL commands to generate the key and signing request::
  ...
  openssl genrsa -out radiant.key 2048
